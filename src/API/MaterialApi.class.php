...
class MaterialApi extends BaseApi
{
     ...
     *  上传图文素材.
     *
     * @param $info 图文素材的信息, title 标题,thumb_media_id 封面图片ID,author 作者,digest 摘要,show_cover_pic 是否显示封面,content 内容支持html少于2万字符 小于1M,content_source_url 点击 阅读原文的url
     *
     ...
     *
     * @param $file  媒体文件路径
     * @param $type 文件类型 (video,image,voice,thumb)
     *
     ...
     */
    public function add($file, $type = "image", $info = array())
    {
        if (!$file || !$type) {
            $this->setError('参数缺失');
            return false;
        ...
        if ($type == "video") {
            ...
            $data = array('media' => '@'.realpath($file), "type" => $type, "description" => $des);
        } else {
            $data = array('media' => '@'.realpath($file), "type" => $type);
        }
     ...
     *   修改永久素材.
     *
     * @param $media_id 素材ID,$info 保存信息,$index 多图文素材时才有意义 更新位置
     *
     * @return bool.  成功返回ok
     */
